...
# Count documents in shard_1_1
log "Counting documents in shard_1_1"
docker compose exec -T shard_1_1 mongosh --port 27018 --quiet <<EOF
use somedb;
db.helloDoc.countDocuments();
EOF

# Count documents in shard_1_2
log "Counting documents in shard_1_2"
docker compose exec -T shard_1_2 mongosh --port 27019 --quiet <<EOF
use somedb;
db.helloDoc.countDocuments();
EOF

# Count documents in shard_1_3
log "Counting documents in shard_1_3"
docker compose exec -T shard_1_3 mongosh --port 27020 --quiet <<EOF
use somedb;
db.helloDoc.countDocuments();
EOF

# Count documents in shard_2_1
log "Counting documents in shard_2_1"
docker compose exec -T shard_2_1 mongosh --port 27021 --quiet <<EOF
use somedb;
db.helloDoc.countDocuments();
EOF

# Count documents in shard_2_2
log "Counting documents in shard_2_2"
docker compose exec -T shard_2_2 mongosh --port 27022 --quiet <<EOF
use somedb;
db.helloDoc.countDocuments();
EOF

# Count documents in shard_2_3
log "Counting documents in shard_2_3"
docker compose exec -T shard_2_3 mongosh --port 27023 --quiet <<EOF
use somedb;
db.helloDoc.countDocuments();
EOF
